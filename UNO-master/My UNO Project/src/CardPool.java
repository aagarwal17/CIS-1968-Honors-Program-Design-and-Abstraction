...
//This class contains the code for all the methods that relate to the deck of cards.
	public class CardPool
	{
		...
		//as the name suggests, this will print out all the cards in the deck to the console
		...
		//as the name suggests, this methods shuffles the cards in the deck through the random method
		...
		//plays the card based on the number input by the user
		public void playCard( int handIndex, ArrayList<UnoCard> playerHand )
		{
			...
			//otherwise, indicate that the card is unplayable
			else
		...
		{
				//Checks if colors match on new card and most recent card played 
	            if(discard.get(0).getColor() == card.getColor()) 
	            {
	                return true;
	            
	            }
	            //Checks face values 
	            else if(discard.get(0).getFace() == card.getFace())
	            {
	                return true;
	            }
